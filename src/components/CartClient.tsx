  ...
  return (
    <div className="grid gap-6">
      <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
        <div className="text-sm font-semibold text-zinc-900">Položky</div>

        {empty ? (
          <div className="mt-4 text-sm text-zinc-500">Košík je prázdný.</div>
        ) : (
          <div className="mt-4 grid gap-4">
            {items.map((it) => (
              <div
                key={it.productId + "-" + (it.decor ?? "") + "-" + (it.felt ?? "")}
                className="flex flex-col gap-4 rounded-2xl border border-zinc-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between"
              >
                <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="inline-flex items-center rounded-xl border border-zinc-200 bg-white">
                    <button
                      className="h-10 w-10 rounded-l-xl text-zinc-900 hover:bg-zinc-50"
                      type="button"
                      onClick={() =>
                        setQty(it.productId, Math.max(0, it.quantity - 1), it.decor, it.felt)
                      }
                    >
                      –
                    </button>
                    <div className="min-w-[52px] text-center text-sm font-semibold text-zinc-900">
                      {it.quantity}
                    </div>
                    <button
                      className="h-10 w-10 rounded-r-xl text-zinc-900 hover:bg-zinc-50"
                      type="button"
                      onClick={() =>
                        setQty(it.productId, it.quantity + 1, it.decor, it.felt)
                      }
                    >
                      +
                    </button>
                  ...
                  <button
                    className="h-10 rounded-xl border border-zinc-200 bg-white px-3 text-sm font-semibold text-zinc-900 hover:bg-zinc-50"
                    type="button"
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
        <div className="text-sm font-semibold text-zinc-900">Poznámka k poptávce</div>

        <textarea
          className="mt-3 w-full rounded-2xl border border-zinc-200 bg-white p-4 text-sm text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-black"
          ...
          <div className="flex items-center gap-3">
            {hasPrices && (
              <div className="text-sm text-zinc-700">
                Celkem: <span className="font-semibold text-zinc-900">{total ?? totalText}</span>
              </div>
            )}
